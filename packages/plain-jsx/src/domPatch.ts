...
export function patchChildren(parent: ParentNode, current: ChildNode[], target: ChildNode[]) {
    const newIndexMap = new Map<ChildNode, number>(target.map((node, index) => [node, index]));
    ...
    // currently both ops are handled the same way, so we are only using one for all.
    interface Op {
        type: 'insert' | 'move';
        pos: ChildNode | null;
        nodes: ChildNode[];
...
function getLIS(arr: number[] | Int32Array): number[] {
    const n = arr.length;
